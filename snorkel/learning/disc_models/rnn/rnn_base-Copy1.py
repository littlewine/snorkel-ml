    ...
    def _build_model(self, dim=50, attn_window=None, max_len=20,
        ...
        **kwargs):
        ...
        if self._embeddings is not None:
            embeddings_initializer = tf.constant_initializer(self._embeddings)
            train_embeddings = False
        else:
            embeddings_initializer = tf.random_normal_initializer(mean=0.0, stddev=1.0)
            train_embeddings = True

        # Embedding layer
        with tf.variable_scope("embedding_layer"):
            embedding_weights = tf.get_variable(name='embedding_weights',
                                                shape=(self._vocab_size, self._embeddings_dim),
