...
############################################
###  Adding a class for linear weighting ###
############################################


class GenerativeModelLinear(Classifier):
    """
    A generative model for data programming for binary classification.
    ...
    def marginals(self, L, candidate_ranges=None, batch_size=None, lf_weights = None, majority_voting=False):
        """
        Given an M x N label matrix, returns marginal probabilities for each
        candidate, depending on classification setting:
        ...
        """
        m, n = L.shape
        if (self.weights is None) and (lf_weights is None) and (not majority_voting):
            raise ValueError("""Must fit model with train() before computing 
                marginal probabilities.""")
        if majority_voting:
            marginals = np.fromiter(map(lambda x: 0.5 if x.item()==0 else(0 if x.item()<0 else 1) ,L.mean(axis=1)), np.float64) # resolving conflicts on 0.5 prob labels
            return marginals
        if (lf_weights is not None): #linear weighting given the observed accuracies on dev set.

            if len(lf_weights)==n:
                lf_weights = np.array(lf_weights)
            elif lf_weights==1:#unweighted average - all classifiers have same weights
                lf_weights = np.ones(n)
            
            else:
                raise ValueError("""Have to give proper weights ==  num LFs""")

            L_weighted = L.multiply(lf_weights)#/lf_weights.sum()
            marginals = np.fromiter(map(lambda x: x.item(),L_weighted.mean(axis=1)), np.float64) #for unweighted scoring (mean value of classifiers)
            # marginals = np.ndarray(L.shape[0], dtype=np.float64)

            return (marginals/2)+0.5 # {-1,1} -> {0,1} mapping

        raise ValueError("""Use the GenerativeModel class instead""")
        ...
        if self.cardinality == 2:
            ...
            for i in range(L.shape[0]):
                ...
                for l_index1 in range(l_i.nnz):
                    data_j, j = l_i.data[l_index1], l_i.col[l_index1]
                    if data_j == 1: # data_j = vote of voter j
                        logp_true  += self.weights.lf_accuracy[j]
                        logp_false -= self.weights.lf_accuracy[j]
                        logp_true  += self.weights.lf_class_propensity[j]
                        logp_false -= self.weights.lf_class_propensity[j]
                    ...
                    for l_index2 in range(l_i.nnz): # only for fixing & reinforcing relationships
                        data_k, k = l_i.data[l_index2], l_i.col[l_index2]
                        if j != k:
                            if data_j == -1 and data_k == 1:
                                logp_true += self.weights.dep_fixing[j, k]
