...
def reload_annotator_labels(session, candidate_class, annotator_name, split, filter_label_split=True, create_missing_cands=False, debug=False):
    ...
    if debug:
        print("AnnotatorLabels not matched to candidates (split=%i): %i"%(split,len(missed)))